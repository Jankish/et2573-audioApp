...
public class StudentDetector implements BabyDetector {
	...
	private final int framesThreshold = 40; //60 ms för att människa ska reagera
	private int K = 150;
	 ...
	 * @return returns the new threshold scaled to the sensitivity level chosen by the user
	 */
	private short getThreshold(short baseline, int percentage) {
		int result = Math.max((int)(((100-percentage)*baseline*K)/100),(int)baseline);
		return (short)result;
	}
